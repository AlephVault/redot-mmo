...
func add_special_scope(id: int) -> Dictionary:
	id = AlephVault__MMO__Common.Scopes.make_fq_special_scope_id(id)
	if _scopes.has(id):
		return _scopes[id]
	else:
...
func _enter_tree() -> void:
	...
	if parent is AlephVault__MMO__Server.Main:
		var server = parent as AlephVault__MMO__Server.Main
...
func _init():
	add_special_scope(AlephVault__MMO__Common.Scopes.SCOPE_LIMBO)
	add_special_scope(AlephVault__MMO__Common.Scopes.SCOPE_ACCOUNT_DASHBOARD)
...
func _unset_connection_from_scope(connection_id: int):
	if _connections.has(connection_id):
		...
		if _scopes.has(scope_id):
			...
			if len(scope) == 0 and AlephVault__MMO__Common.Scopes.ScopeType.get(scope_id >> 30) != AlephVault__MMO__Common.Scopes.ScopeType.SPECIAL:
...
func set_connection_scope(connection_id: int, scope_id: int):
	...
	var node: AlephVault__MMO__Server.Connection = get_connection_node(connection_id)
	if node:
...
func get_connection_node(id: int) -> AlephVault__MMO__Server.Connection:
...
# connection id.
func _add_client(id: int) -> AlephVault__MMO__Server.Connection:
...
func _remove_client(id: int):
	...
	if node:
		...
		(get_parent() as AlephVault__MMO__Server.Main).scope_changed.emit(node.id, -1, -1)
		remove_child(node)
